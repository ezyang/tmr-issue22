...
\EZY{I rewrote the abstract, let me know if you like it.}
\Matt{LGTM, thanks!}
...
\EZY{Fun fact: it is possible to create parsers which only have an applicative instance,
and are more efficient but less expressive than the style you have here!}
\begin{verbatim}
...
\EZY{Do you maybe want to say why this is the case?}
...
MonadState constraint in the type signature.  The mtl \EZY{which?} \Matt{I'm not sure what the library's called -- is it mtl or transformers?} formulation of monad
...
\EZY{An alternate structure you can use here is to introduce Woof first, before talking about monad transformers.  After all, the example parser doesn't require you to know what is going on in transformer-land.}
...
clearly indicate their semantics: \EZY{I would suspect many compiler authors would be of the opinion that the proper way to remove comments and whitespace is to lex first, and parse later.}
\Matt{That's a good point, I toyed with doing it that way but it was just too much trouble for no benefit that I could see.  It ended up being a lot more code.
But this approach is equivalent, since the token parsers get the first crack at the input string, and the non-token parsers don't touch the input string directly.}
\begin{verbatim}
many0 = many
many1 = some
...
parses the input, or the parsing stops.
...
\EZY{It's worth mentioning why, formally speaking, this is the case, which has
to do with the relevant structures being isomorphic to each other.}
\Matt{Uh-oh!  I don't really know anything formal about monad transformers,
I really have no idea how to go about showing something like this!
I guess I'll have to rethink my goals for this section.}

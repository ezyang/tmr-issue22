...
\EZY{Question a reader might have: what is the difference between failure and error?  
You give a little guidance later but give some intuition now!}
...
A quick word about Parsec \cite{leijen}, an important, battle-tested parser 
combinator library for Haskell.

One major difference between the approach I've described and Parsec's approach
is with respect to backtracking and error-reporting.  Parsec's default is 
predictive parsing using a single token of lookahead for efficiency.  This can
be changed using its \verb+try+ combinator.

On the other hand, my approach by default uses unlimited lookahead, which can 
be modified using the \verb+throwError+ and \verb+commit+ combinators.

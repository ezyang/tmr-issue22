...
build parsers, we'll first create a minimal parser combinator library.
Next, we'll add monad transformers to the mix.  We'll divide the library
into four main pieces:

\begin{itemize}
 \item the Parser datatype,
 \item a primitive parser,
 \item type class instances, and
 \item combinators.
\end{itemize}
...
We also want parsers to return a value, if they succeed.  To do this, we extend the return type
to allow for a value, which also requires a second type parameter:
\begin{verbatim}
newtype Parser t a = 
...
is run, then the second is run, and the token position is threaded between the two.
...
Just ("(()abc(def)()",[])
\end{verbatim}
This concludes the implementation of a simple parser.
...
This is identical to the Parser type given before modulo the order 
of the result tuple, so we can redefine the parser type as:
\begin{verbatim}
...
\begin{itemize}
 \item compositional implementation with semantic building blocks --
    the mtl library provides implementations
    for Maybe, Error, and State building blocks; using these, we can easily
    describe the semantics of our parsers without writing very much code, and
 \item type class instances -- in addition to providing the building blocks,
    ...
    implement parser-specific operations.
\end{itemize}
...
The rest of this articule will progressively adding features to a simple 
initial implementation on our way to creating an error-reporting parser.
The simple language we'll use as a motivation for this task is Woof, a 
...
There is an additional rule that whitespace and comments may appear in any 
amount before or after tokens.  Our tokens are:
\begin{itemize}
  \item \verb+{+
  \item \verb+}+
  \item \verb+(+
...
Our first Woof parser will only be responsible for determining whether an input
...
Recall, however, that our specification said that comments and whitespace can occur 
anywhere before or after tokens -- how should this be handled?

First, we need to define the whitespace and comment patterns -- we'll also rename
...
We want to throw away comments and whitespace, and this process
must occur after every token.  We can do this by using
a combinator which first parses a token and then throws away junk:
\begin{verbatim}
...
With \verb+tok+ in hand, we can fix \verb+opencurly+:
\begin{verbatim}
...
otherwise, and discards trailing whitespace and comments.
...
The parser for our last token, Symbol, is more complicated, as any number of
...
The first combining form is function application.  The rule says that it is delimited
...
our new parsers have three possible results:  success (Right Just), failure (Right Nothing), and error (Left).
%% EZY
...
We'll implement our own MonadError type class.  We'll need an instance for each member of
...
both require that the next lower level supports MonadError.  This code uses
type families to state what the error type is; explaining type families
in more detail is out of scope for this article, but roughly, given some MonadError \verb|m|,
you can write down the type of error that monad supports by saying \verb|Error m|.
...
data should it include?  \verb+throwError+ will unconditionally throw an
error, but often we will only want to conditionally throw an error. Thus, we
...
combinator of \cite{hutton1992higher} and also on \cite{Mizushima}.
...
up (fortunately, because the types of the states are different, Haskell's type system will helpfully complain if I mess up).
...
We could see these advantages when many of the parsers didn't have to change 

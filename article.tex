...
MonadState constraint in the type signature.  The mtl \EZY{which?} \Matt{I'm not sure what the library's called -- is it mtl or transformers?} \EZY{Well, both export the necessary interfaces. Just pick one and be consistent.} formulation of monad
...
\EZY{It's pretty simple, and just has to do with the fact that Maybe (Either a b) is
equivalent (modulo laziness) with Either a (Maybe b).  To show this is a case, you have
to define functions which convert from one to the other, and then show that the roundtrip
is identity.}

...
There are such many excellent papers for reading about parser combinators, such
as this \cite{wadler}, this \cite{hutton}, and this \cite{leijen}.  In fact, the
inspiration and terminology for much of this article is drawn directly from
those references.
...
Parsers built out of \verb+literal+ only succeed when the next token matches the given
one:
\begin{verbatim}
...
Monad transformers \cite{liang} allow monads to be modularly combined to form new monads
...
Now we can build the \verb+item+ parser using the combinators from MonadState, 
since its token lists are the 'state' that it modifies.
\begin{verbatim}

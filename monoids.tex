...
:: [Task] -> Schedule
\end{spec}
Any time I see see a function of this form, I ask myself, ``Can it be implemented using the \h{HomTrainer} type class?'' \EZY{HomTrainer is not in the base library, so it's worth mentioning where you can get it from.  You did say so in the abstract (it's HLearn), but it's worth repeating here, with a cite.}
...
\EZY{This is a funny claim to make. The conventional definition of a vector is an nat-indexed
data type, i.e. the fixed sized lists.  So if you try to define a Monoid instance on them,
you'll find that you can't get the types to line up, because the size changes when you append.
You could say, ``Well, these are vectors where the size is unknown'', but that is isomorphic
to list, so there's no point in mentioning them.}
...
% allStudents::[Student]
% \end{spec}
using the \h{Cassava} package.\EZY{cite?}

...
This article is a tutorial about one such extension -- generalized algebraic data types (GADTs). The single idea of GADTs is to allow specific return types of value constructors. \EZY{I think this explanation doesn't mean anything to someone who doesn't already know what GADTs are.} In this way they generalize ordinary algebraic data types. The theoretical foundation for GADTs is the notion of dependent types which is extensively described in literature on computer science and logic~\cite{pierce_book}.
...
\EZY{I think I'd add something like, ``While creating a values with the TI results in a value of type Test Int, the converse holds too: when a pattern-match succeeds on the TI constructor, we can now conclude that a is Int; that is to say, pattern matching causes type refinement.''}
...
% TODO describe other ways to implement Plus?
...
\EZY{Yes, please write a conclusion here!}

%The moral is that to verify a function it is required to carefully design data type.
...
\EZY{Before you dive into all of the hoary details of the construction, it is well worth motivating the example. What's the point? Answer this question!}
...
\item Every leaf is black. % (this is enforced by the tree data type).
...
\EZY{This section is something of a digression from the litany of examples that have been parading down this article; so it's probably worth pointing it out and mentioning why this is an important thing to know about.}
...
\EZY{What are the limitations of finally tagless in this context?}
...
% Comparing to an approach to make verification by translating Haskell to other languages and back (such as xmonad), verification in Haskell itself...

As a result, we have shown that the notion of GADTs is a very valuable extension of the Haskell language.

\EZY{I think you can write a better concluding sentence then that!}
